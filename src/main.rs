...
pub struct ParserOptions {
    ...
    /// Restarts processing from genesis. Ignoring previously processed blocks,
    restart: bool,
...
fn main() {
    ...
    debug!(target: "main", "Using LogLevel {}", options.log_level);
    if options.reindex {
    ...
    for i in 0..iterations {
        ...
        let mut chain_file = match load_chain_file(&options.chain_storage_path) {
            Ok(f) => f,
            Err(desc) => {
...
fn parse_args() -> OpResult<ParserOptions> {
    ...
    let matches = App::new("Multithreaded Blockchain Parser written in Rust")
        ...
        .arg(Arg::with_name("reindex")
            ...
            .help("Force complete reindexing. Removes chain.json"))
        .arg(Arg::with_name("restart")
            .short("s")
            .long("restart")
            .conflicts_with("resume")
            .help("Ignores previously processed blocks and starts from scratch without re-indexing"))
        ...
        .arg(Arg::with_name("max-height")
            ...
            .long("max-height")
            .value_name("COUNT")
            .help("Number of blocks to parse")
            .takes_value(true))
    ...
    let max_height = value_t!(matches, "max-height", usize).ok();
